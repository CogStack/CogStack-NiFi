...
// only include these metadata fields
//
Set includeMetaFields = (include_metadata_fields as String).split(',').collect { it.trim().toLowerCase() }
...
flowFile = session.write(flowFile, { inputStream, outputStream ->
	...
	rec.each {k, v ->
		if (!k.equals(document_text_field as String) && includeMetaFields.contains(k))
        	footer.put(k, v)
    	}
    	outRec = [:]
