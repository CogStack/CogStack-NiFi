...
def inJson = jsonSlurper.parseText(flowFileContent)
  ...
  inJson.result.each { res ->
    ...
    annotations.each { ann ->
       def outAnn = [:]
        ...
        // get the fields from the footer
        result_json.footer.each { k, v ->
            if (k.equals(document_id_field) || k == "id") {
              outAnn.put(FIELD_META_PREFIX + k, v)
            }
        }
      // filter the ignored annotations (by type)
      def annotationTypes = ann.collect { it.value.types.collect { it.trim().toLowerCase() } }
      ...
      def annValue = ann.collect{it.value}

      annValue.each{ individual_ann ->
        def annData = individual_ann.collect{it}
        annData.each{ data ->
            outAnn.put(FIELD_NLP_PREFIX + data.getKey(), data.getValue())
        }
        ...
        def annotationIdField = FIELD_NLP_PREFIX + (annotation_id_field as String)

        def doc_id = outAnn[FIELD_META_PREFIX + "id"]
        def ann_id = outAnn[annotationIdField]
        ...
        outAnn["document_annotation_id"] = doc_ann_id
