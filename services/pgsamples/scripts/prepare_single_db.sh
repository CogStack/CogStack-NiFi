...
# global variables
#
POSTGRES_USER_SAMPLES=${POSTGRES_USER_SAMPLES:-"test"}
POSTGRES_PASSWORD_SAMPLES=${POSTGRES_PASSWORD_SAMPLES:-"test"}
DB_USER=$POSTGRES_USER_SAMPLES
DB_PASSWORD=$POSTGRES_PASSWORD_SAMPLES
DB_NAME='db_samples'
...
error_handler() {
  ...
  rm -rf $TMP_DIR
  rm -rf $SQL_FILE
  psql -v ON_ERROR_STOP=0 -U $DB_USER -c "DROP DATABASE $DB_NAME"
  psql -v ON_ERROR_STOP=0 -U $DB_USER -c "DROP ROLE $DB_USER;"
  exit 1
}
...
rm -r $TMP_DIR
rm -r $SQL_FILE
psql -v ON_ERROR_STOP=1 -U $DB_USER -c "DROP DATABASE $DB_NAME"
psql -v ON_ERROR_STOP=1 -U $DB_USER -c "DROP ROLE $DB_USER;"
